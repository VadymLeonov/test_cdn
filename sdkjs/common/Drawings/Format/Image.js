...
CImageShape.prototype.draw = function(graphics, transform)
{
    ...
    if(this.getObjectType() === AscDFH.historyitem_type_OleObject){
        ...
        if(typeof graphics.drawField === "function") {
            var dimensionMultiplier = 3.78;
            var OleObjectDataExists = typeof this.m_sData !== "undefined" && this.m_sData.length > 0;
            if(OleObjectDataExists) {
                var FieldJSONisValid = true;
                try {
                    var OleObjectData = JSON.parse(this.m_sData);
                } catch (e) {
                    FieldJSONisValid = false;
                }
                var fieldIdExists = FieldJSONisValid && typeof OleObjectData.internalFieldId !== "undefined" && OleObjectData.internalFieldId.length > 0;
                if(fieldIdExists) {
                    var coordinates = {
                        id: this.m_oSectPr.Id.split('_')[1],
                        pageNumber: graphics.m_lPagesCount,
                        x: this.posX * dimensionMultiplier,
                        y: this.posY * dimensionMultiplier,
                        width: this.bounds.w * dimensionMultiplier,
                        height: this.bounds.h * dimensionMultiplier,
                    };
                    var fieldJson = JSON.stringify(Object.assign(OleObjectData, coordinates));
                    graphics.drawField(fieldJson);
                    if (oClipRect) {
